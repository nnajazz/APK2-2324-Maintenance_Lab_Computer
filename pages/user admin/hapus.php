...
$sql = "SELECT `tbl_admin`.*, `tbl_users`.* FROM `tbl_admin` 
	LEFT JOIN `tbl_users` ON `tbl_admin`.`id_user` = `tbl_users`.`id_user` WHERE tbl_users.id_user='$id' ";
                                ...
                                <div id="defaultModal2" modal-center class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
                                    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
                                        ...
                                        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                                            <p class="text-slate-500 dark:text-zink-200">Apakah anda yakin akan menghapus admin <span style="color: red;"><?php echo $row ['nama_admin']; ?></span></p>
                                                <div class="flex justify-end gap-2">
                                                <a href="?pages=user_admin" type="reset"  class="text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 dark:ring-custom-400/20">Cancel</a>

                                                <a href="?pages=user_admin&aksi=proses_hapus&id=<?php echo $row['id_user'];  ?>" type="submit" name="Hapus" class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100">Hapus</a>
                                        </div>
